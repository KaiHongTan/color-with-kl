...
import { Col, Container, Popover, Row , Overlasephorarigger, Tooltip} from 'react-bootstrap';
...
const Shopping = () => {
  ...
  function changePic(food){
    ...
    else if (((pavilliondone===1 && sephoradone===0))||((pavilliondone===0 && sephoradone===1))){
      setImage(all);
      checkFood(food);
      setalldone(true);
    }
    ...
    <div className="App2">
      ...
      <div>
        ...
        <div className="palettes">
                {showpavillion ? <div className="color-swatch" draggable="true" onDragStart={(event)=>dragpavillion(event)} style={{backgroundColor: 'salmon'}}></div> :null}
                {showsephora ? <div className="color-swatch"  draggable="true" onDragStart={(event)=>dragsephora(event)} style={{backgroundColor: 'steelblue'}}></div> :null}
        </div>
